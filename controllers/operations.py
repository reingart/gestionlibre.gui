...
def on_movements_start_submit(evt):
    if session.form.accepts(evt.args, formname=None, keepvalues=False, dbio=False):
        ...
        config.html_frame.window.action(URL(a="gestionlibre", c="operations", f="movements_header"))


def movements_start(args=[], vars={}):
...
def on_movements_header_submit():
    if session.form.accepts(evt.args, formname=None, keepvalues=False, dbio=False):
        print "Form vars"
        print dir(session.form.vars)
        
        if operation.type in ("S", "P"):
            xml = config.html_frame.window.action(URL(a="gestionlibre", c="operations" ,f="movements_price_list"))
        else:
            xml = config.html_frame.window.action(URL(a="gestionlibre", c="operations" ,f="movements_detail"))
        print "SetPage"
        print xml
        config.html_frame.window.SetPage(xml)

def movements_header(args=[], vars={}):
    ...
    if form.accepts(request.vars, session):
        if operation.type in ("S", "P"):
            return dict(_redirect=URL(f="movements_price_list"))
        else:
...
def movements_detail(args=[], vars={}):
    ...
    return dict(operation = BEAUTIFY(operation), \
    movements = BEAUTIFY(movements), price_list = price_list, \
...
def movements_current_account_concept():
    ...
    if form.accepts(request.vars, session, formname="concept_form"):
        ...
        return dict(_redirect=URL(f="movements_current_account_quotas"))
...
def movements_current_account_quotas():
    ...
    formname="quotas_number_form"):
        ...
        return dict(_redirect=URL(f="movements_current_account_data"))
    return dict(form = form)
...
def movements_select(args = [], vars = {}):
